...
public class ChordCreatorUtil {
    ...
    protected void getChords(int[] tuning,
        ...
        count = 0;
        for (int requiredNote : this.requiredNotes)
            if (requiredNote != -1) {
                tempNotes[count] = requiredNote;
                count++;
            }
        // and substitute them
    ...
    protected int checkForOverlapping(int checkIt) {
        for (int expandingNote : this.expandingNotes)
            if (expandingNote == checkIt)
                return -1;
        return checkIt;
    }
    ...
    private List<List<StringValue>> makeCombinations(List<List<StringValue>> potentialNotes) {
        ...
        for (List<Integer> currentStringCombination : stringCombination) { // go through all string combinations list
            // take a string combinations
            List<List<StringValue>> res = null;

            // go through all the strings in one combination
            for (Integer aCurrentStringCombination : currentStringCombination) {

                // take the string index
                int currentString = aCurrentStringCombination;

                // take all the potential notes from currentString and combine
                // them with potential notes from other strings

                res = makeStringValueCombination(res, potentialNotes.get(currentString));
     ...
     *
     * @param lastLevelCombinationRef structure to be expanded by current level
    ...
    private List<List<Integer>> makeStringCombination(List<List<Integer>> lastLevelCombinationRef) {
        ...
        List<List<Integer>> thisLevelCombination = new ArrayList<List<Integer>>();
        for (int current = 1; current < this.tuning.length; current++) {
            ...
            for (List<Integer> combination : lastLevelCombination) {
                ...
                if (combination.get(combination.size() - 1) < current
    ...
    private List<List<StringValue>> makeStringValueCombination(List<List<StringValue>> lastLevelCombination, List<StringValue> notes) {
        ...
        List<List<StringValue>> thisLevelCombination;
        ...
        if (lastLevelCombination == null) { // initial combination
            ...
            for (StringValue note : notes)
                for (List<StringValue> aLastLevelCombination : lastLevelCombination) { // cartesian multiplication
                    List<StringValue> currentCombination = new ArrayList<StringValue>(aLastLevelCombination);
                    currentCombination.add(note);
    ...
    private boolean checkCombination(List<StringValue> combination) {
        ...
        for (StringValue aCombination : combination) {

            int fret = aCombination.getFret();
        ...
        return Math.abs(maxLeft - maxRight) < MAX_FRET_SPAN;
    ...
    private List<PriorityItem> determinePriority(List<List<StringValue>> allCombinations) {
        ...
        for (List<StringValue> stringValueCombination : allCombinations) {
            if (isValidProcess()) {
                break;
            }
    ...
    private float combinationHasAllRequiredNotes(List<StringValue> stringValueCombination) {
        ...
        if (currentIndex == this.requiredNotes.length - 1) {
            ...
            if (!existsSubdominant && currentIndex == this.requiredNotes.length - 1) {
                ...
                if (ChordDatabase.get(this.chordIndex).getName().contains("sus") && this.requiredNotes.length != 2 && this.add5 == 0) {
    ...
    private float combinationHasSubsequentStrings(List<StringValue> stringValueCombination) {
        ...
        for (int i = 0; i < this.tuning.length; i++) {
            ...
            for (StringValue aStringValueCombination : stringValueCombination)
                if ((aStringValueCombination).getString() == i)
                    found = true;
            if (found) {
                if (!stumbled)
                    stumbled = true;
    ...
    private float combinationHasGoodFingering(List<StringValue> stringValueCombination) {
        ...
        switch (Math.abs(max - min)) {
            case 0:
                distanceGrade = ChordSettings.instance().getFingeringGrade() / 5;
                break;
            case 1:
                distanceGrade = ChordSettings.instance().getFingeringGrade() / (5 + maxCount);
                break;
            case 2:
                distanceGrade = ChordSettings.instance().getFingeringGrade() / (6 + maxCount);
                if (min < 5) distanceGrade *= 0.9;
                break;
            case 3:
                distanceGrade = -ChordSettings.instance().getFingeringGrade() / 10 * maxCount;
                // I emphasize the penalty if big difference is on some
                // lower frets (it is greater distance then)
                if (min < 5) distanceGrade *= 1.3;
                break;
            case 4:
                distanceGrade = -ChordSettings.instance().getFingeringGrade() / 4 * maxCount;
                if (min <= 5) distanceGrade *= 1.8;
                break;
            default:
                distanceGrade = -ChordSettings.instance().getFingeringGrade() * maxCount;
                break;
        }
        ...
        List<Integer>[] fingers = new List[]{new ArrayList<Integer>(2), new ArrayList<Integer>(2), new ArrayList<Integer>(2), new ArrayList<Integer>(2)};
...
*/
    ...
    private float combinationChordSemantics(List<StringValue> stringValueCombination) {
        ...
        for (int string = 0; string < this.tuning.length; string++) {
            ...
            for (StringValue sv : stringValueCombination) {
                ...
                if (sv.getString() == string && sv.getFret() != -1) { // stumbled upon next string
                    current = sv;
                    found = true;
                    stringDepth++;
                }
        ...
        if (this.alteration != 0) {
            ...
            for (int aFoundExpanding : foundExpanding) {
                if (aFoundExpanding == this.ESSENTIAL_INDEX)
                    essentials++;
                else if (aFoundExpanding != 0)
                    nonEssentials++;
            }
    ...
    private boolean checkIfSubset(List<StringValue> stringValues, List<List<StringValue>> betterOnes) {
        ...
        for (List<StringValue> currentStringValue : betterOnes) {
            ...
            for (StringValue aCurrentStringValue : currentStringValue) {
                int currentString = aCurrentStringValue.getString();
                // search for the same string - if not found do nothing
                for (StringValue stringValue : stringValues)
                    if (stringValue.getString() == currentString) {
                        // if the frets on the same string differ, then chords are not subset/superset of each other
                        if (stringValue.getFret() != aCurrentStringValue.getFret())
